...
def main():
    ...
    coverage = 0
    length = 0
    simpleEdge = newEdge.copy()
    for i in newEdge:
        coverage += newEdge[i]
        length += len(i)
    coverage = coverage * 1.0 / len(newEdge)
    length = length * 1.0 / len(newEdge)
    for i in newEdge:
        if newEdge[i] < coverage / 2 and len(i) < 2 * k:
            del simpleEdge[i]
    with open('deBruijnGraphSimpleEdge_s_6.first100000.fasta', 'w') as dBE:
        for i in simpleEdge:
            dBE.write('[' + str(i) + ',' + str(simpleEdge[i]) + ']\r\n')
    with open('deBruijnGraphQUAST_s_6.first100000.fasta', 'w') as dBE:
        for i in simpleEdge:
            dBE.write('>edge from ' + str(i[:k]) + ' to ' + str(i[-k:]) + '\r\n' + str(i) + '\r\n')
    with open('deBruijnGraphSimplification_s_6.first100000.dot', 'w') as dBC:
        dBC.write('digraph G{\r\n')
        for i in simpleEdge:
            #dBC.write(' ' + str(i[:k]) + '->' + str(i[-k:]) + ' [label=' + i + '];\r\n')
            dBC.write(' ' + str(i[:k]) + '->' + str(i[-k:]) + ';\r\n')
